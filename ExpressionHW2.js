...
class Binary {
   ...
   class ikiParametre{ //yeni ekledik sor bunu
	   constructor(left, oper, right) {
	   this.left = left; this.oper = oper; this.right = right; 
	   }
	   fValue() {
...
class tekParam { // bu kısım tek parametre alan Math fonksiyonları için //yeni
...
function factor() {
  switch (tok.kind)  {
    ...
    ciftParametreAlan= F.filter(k => Math[k].length==2) ; //iki parametre alanlar
